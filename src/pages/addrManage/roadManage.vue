<template>
    ...
    <el-dialog v-model="modalState" :title="modalTitle"  center>
        ...
        <el-form-item label="所属村庄" required>
            ...
            <el-option v-for="item in villageList" :key="item.zoneId" :label="item.village" :value="item.zoneId"></el-option>
            </el-select>
        </el-form-item>
        </el-form>
        <template #footer>
...
<script setup>
 ...
 import {fetchCityList,fetchVillageList,fetchDistrictList,fetchTownList,fetchRoadList,AddRoadInfoByConfig,editRoadInfoByConfig,deleteRoadInfoById} from '../../apis/2.0/addr'
 let roadList=ref([])
 let villageList=ref([])
 let townList=ref([])
