    ...
    <el-dialog v-model="modalState" :title="modalTitle" style="font-weight: 500">
      <el-form :model="form" label-width="100px" :inline="false">
        ...
        <el-form-item label="API 地址" required>
          <el-input v-model="form.link" :disabled="mode === 'edit' || mode === 'normal'"
...
const validateSort = sort => {
  ...
  .filter(
      item => (mode.value === 'edit' ? item.backendApiId !== currentItem.value.backendApiId : true))
...
const validate = () => {
  if (!form.desc || !form.sort || (!form.method && mode.value !== 'normal') || !form.name
      || !form.link) {
    ElMessage({
      type: 'info',
      message: '必须项不能为空',
