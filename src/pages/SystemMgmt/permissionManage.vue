<template>
  <div class="p-page">
    <div class="p-header">
      <div class="p-name">
        <i @click="goBack" class="el-icon-back back" style="position:relative; font-size:30px; top:32px; cursor:pointer;"></i>
        <p class="page-name">编辑岗位权限</p>
        <p style="position:relative; margin-left:50px; margin-top:15px;">当前岗位：{{ currentItem.roleName }}</p>
      </div>
        <!-- <div class="p-row">
        <div class="op-flex">
          <el-input class="margin-right" placeholder="乡镇名称"></el-input>
          <el-button type="primary">查询</el-button>
        </div>
      </div> -->
        <div class="button">
          <el-button icon="el-icon-plus" @click="permissionFunc.selectAll" style="margin-left:-220px">全选</el-button>
        ...
        <el-table-column fixed="right" label="选择权限" width="400">
          <template #default="scope">
            <el-button
              ...
              >已选择</el-button
            >
            <el-button v-if="scope.row.roleId != id" icon="el-icon-plus" @click="permissionFunc.selectRow(scope.row)"
              >未选择</el-button
            >
          </template>
        </el-table-column>
...
<script setup>
  ...
  const permissionFunc = {
    async search() {
      const r = await fetchMenuListById({ id: id.value })
    ...
    async save() {
      ...
      const res = await saveMenuByList({
        id: id.value,
