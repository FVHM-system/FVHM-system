...
<div class="box">
  <div class="boxContent">
    ...
    <div class="boxInner">
        <div class="inputRow">
        <el-input class="inputSingle" type="text" v-model="loginForm.username" placeholder="用户名"/>
        </div>
        <div class="inputRow">
        <el-input class="inputSingle" type="password" v-model="loginForm.password" placeholder="密码"/>
        </div>
    ...
    login () {
      ...
      if (this.loginForm.username === '' || this.loginForm.password === '') {
        ...
        axios.request({
          ...
          if (res.data.data.code=='200') {alert('登陆成功');}
        }).catch(error => {
          alert('账号或密码错误');
          console.log(error);
        });
...
<style lang="scss" scoped>
  * {
  ...
  .boxConfirm {
    background: rgba(66, 62, 255, 0.93);
    height: 47.5px;
    width: 142.5px;
    color: rgba(255, 255, 255, 0.9);
  ...
  .choose {
    width: 380px;
    border: solid 0.95px rgba(0, 0, 0, 0.1);
    border-radius: 4.75px;
    overflow: hidden;
  }
