...
<script setup>
  ...
  import { addMenuByConfig, deleteMenuById, editMenuByConfig, fetchMenuListByNone } from '/src/apis/2.0/menu.js'
 ...
 const id = computed(() => route.query.id)
  ...
  const validateSort = sort => {
    const res = originData
      .filter(item => (mode.value === 'edit' ? item.frontendMenuId !== currentItem.value.frontendMenuId : true))
